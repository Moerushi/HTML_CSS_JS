// заготовка для игры из главы 16 (часть 2)

class CanvasDisplay {
    constructor(parent, level) {
        this.canvas = document.createElement.createElement('canvas');
...
CanvasDisplay.prototype.updateViewport = function (state) {
    ...
    if (center.y < view.top + margin) {
        ...
        view.top = Math.min(center.y + margin - view.height, state.level.height)
    }
}

CanvasDisplay.prototype.clearDisplay = function (status) {
    if (status == 'won') {
        thix.cx.fillStyle = 'rgb(68,191,255)';
    } else if (status == 'lost') {
        thix.cx.fillStyle = 'rgb(44,136,214)';
    } else {
        thix.cx.fillStyle = 'rgb(52,166,251)';
    }
    this.cx.fiilRect(0, 0, this.canvas.width, this.canvas.height);
}
