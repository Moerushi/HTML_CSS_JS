...
console.log(filter(SCRIPTS, script => script.living)); // где script это элемент foreach, возращает то, что после =>, в данном случае false или true
console.log(SCRIPTS.filter(s => s.direction == "ttb")); // где s это элемент foreach // при этом создается именно новый массив
...
let rtlScripts = SCRIPTS.filter(s => s.direction == "rtl"); // создает массив фильтрованных элементов для письма справа налево
console.log(map(rtlScripts, s => s.name)); // возвращает измененный массив по условию s => s.name, которое возвращает имя элемента массива
...
console.log(reduce([1, 2, 3, 4], (а, b) => а + b, 0)); // берет первый элемент и через функцию (а, Ь) => а + Ь складывает текщуий и следующий элемент
...
function characterScript(code) {
  for (let script of SCRIPTS) {
    if (script.ranges.some(([from, to]) => code >= from && code < to)) { // проверяет на true или false
      return script;
    }
  }
  return null;
...
let chineseCharacters = "秘书";
console.log(chineseCharacters.length);
console.log(chineseCharacters[0]); // вывод элемента
console.log(chineseCharacters.charCodeAt()); // выводит значение юникода (не полный код)
console.log(chineseCharacters.codePointAt(0)); // выводит значение юникода (полный код)

function countBy(items, groupName) {
  let counts = [];
  for (let item of items) {
    let name = groupName(item);
    let known = counts.find(c => c.name == name); // вернёт первый найденный в массиве элемент, который подходит под условие в переданной колбэк-функции
    if (!known) {
...
console.log(countBy([4, 2, 3, 4, 5], n => n > 2)); // принимает коллекцию, возвращает массив обектов, каждый из которых содержит имя группы кол-во найденных элементов
...
console.log(textScripts('英国狗说 woof, 俄罗斯狗说 тяв'));
