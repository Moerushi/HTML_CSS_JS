...
export const UniqueProdCard = ({ imgSrc, colors, sizes, price, title, description, category }) => {

  const dispatch = useDispatch();

  const [prodColor, setColor] = useState(colors[0]);
  const [prodSize, setSize] = useState(sizes[0]);
  const [quantity, setQuantity] = useState(1);
  const [chosenColor, setChosenColor] = useState(true);
  const [chosenSize, setChosenSize] = useState(true);
  ...
  const handleChosenColorChange = ({ target }) => {
    setChosenColor(target.value);
  }

  const handleChosenSizeChange = ({ target }) => {
  ...
  return (
    <div className="product-description__box center">
      <div className="product-description">
        <h2 className="product-description__title">{category} COLLECTION</h2>
        <div className="product-description__topline"></div>
        <h2 className="product-description__product-name">{title}</h2>
        <p className="product-description__product-text">{description}</p>
        <p className="product-description__product-price">${price}</p>
          ...
          <details className="product-description__product-options__item">
            ...
            <div className="product-description__product-options__choise">
              ...
              {colors.map((color, index) => (
                <label onChange={handleColorChange} key={index} className="product-description__product-options__label" htmlFor={color.toLowerCase()}>
                  <input onChange={handleChosenColorChange} checked={color === prodColor ? chosenColor : false} className="option-input" type="radio" name="color" value={color}
          ...
          <details className="product-description__product-options__item">
            ...
            <div className="product-description__product-options__choise">
              {sizes.map((size, index) => (
                <label onChange={handleSizeChange} key={index} className="product-description__product-options__label" htmlFor={`${size.toLowerCase()}-size`}>
                  <input onChange={handleChosenSizeChange} checked={size === prodSize ? chosenSize : false} className="option-input" type="radio" name="size" value={size}
          ...
          <details className="product-description__product-options__item">
            ...
            <div className="product-description__product-options__choise">
              <label className="product-description__product-options__label" htmlFor="quantity">
                <input onChange={handleQuantityChange} className="product-description__product-options__quantity" type="number" name="quantity" step="1"
                  value={quantity} min="1" max="10" id="quantity" />
              </label>
            </div>
          </details>

        </div>
        <div onClick={() => dispatch(addToCart({ title, prodColor, price, prodSize, quantity, imgSrc }))} className="product-description__addtocart">
